...
// Use the Flask backend on port 5001 unless the UI is already served from that port.
const API_BASE = (() => {
    if ((window.location.protocol === 'http:' || window.location.protocol === 'https:') && window.location.port === '5001') {
        return window.location.origin;
    }

    const host = window.location.hostname || 'localhost';
    return `${window.location.protocol === 'https:' ? 'https:' : 'http:'}//${host}:5001`;
})();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    generateBtn.addEventListener('click', async (e) => {
        ...
        try {
            ...
            const msg = 'Could not connect to the try-on server. Make sure the Python backend (tryon_pipeline.py) is running on port 5001.';
            showError(msg);
            botSay(`❌ ${msg}`);
        } finally {
            generateBtn.disabled  = false;
